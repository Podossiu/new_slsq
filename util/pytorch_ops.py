...
def linear_flops_counter_hook(module, input, output):
    ...
    module.__bops__ += module.__flops__ * 32 * 32

def pruned_linear_flops_counter_hook(module, input, output):
    input = input[0]
    output = output[0]
    '''
    input = input[0]
    ...
    bias_flops = output_last_dim if module.bias is not None else 0
    module.__flops__ += int(np.prod(input.shape) * output_last_dim + bias_flops)
    module.__flops__ -= (m.weight == 0).float().sum() * 2
    '''
    output_last_dim = output.shape[-1]
    bias_flops = output_last_dim if module.bias is not None else 0
    
    density = (module.weight != 0).float().sum() / module.weight.numel()
    linear_flops = (np.prod(input.shape) * output_last_dim * density).item()
    module.__flops__ += int(linear_flops + bias_flops)
    if hasattr(module, "quan_w_fn") and hasattr(module.quan_w_fn, "bit"):
        module.__bops__ += int(module.__flops__) * module.quan_w_fn.bit * (module.quan_a_fn.bit+1)
    else:
        module.__bops__ += int(module.__flops__) * 32 * 32
...
def conv_flops_counter_hook(conv_module, input, output):
    ...
    conv_module.__bops__ += int(overall_flops) * 32 * 32

def pruned_conv_flops_counter_hook(conv_module, input, output):
    input = input[0]
    output = output[0]
    batch_size = input.shape[0]
    output_dims = list(output.shape[2:])
    ...
    density = (conv_module.weight != 0).float().sum() / conv_module.weight.numel()
    print(density)
    overall_conv_flops = (overall_conv_flops * density).item()

    overall_flops = overall_conv_flops + bias_flops

    conv_module.__flops__ += int(overall_flops)
    if hasattr(conv_module, "quan_w_fn") and hasattr(conv_module.quan_w_fn, "bit"):
        conv_module.__bops__ += int(overall_flops) * conv_module.quan_w_fn.bit * (conv_module.quan_a_fn.bit+1)
    else:
        conv_module.__bops__ += int(overall_flops) * 32 * 32
...
MODULES_MAPPING = {
    ...
    QuanLinear : pruned_linear_flops_counter_hook,
    QuanConv2d : pruned_conv_flops_counter_hook,
}
